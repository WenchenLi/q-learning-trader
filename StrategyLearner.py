...
class StrategyLearner(object):
  ...
  def addEvidence(self, symbol="SPY",
    ...
    # Instantiate a Q-learner
    self.learner = ql.QLearner(num_states=self.num_state, num_actions=self.num_action,rar=.5,alpha=.001,alpha_decay=.99, dyna=self.learner_dyan_iter)
    ...
    for k in xrange(self.num_epoch):
      ...
      if self.verbose:print "epoch",k," current cumulated return:", self.get_current_portfolio_values(price_array[-1])/float(sv) - 1.0,"portfolio:",self.portfolio
      self.cumulated_return_indicator = self.last_cumulated_return/(self.get_current_portfolio_values(price_array[-1]) / float(sv))
    ...
    ut.plot_data(trades, title="transactions_train", ylabel="amount", save_image=True,save_dir=self.current_working_dir)
    ut.plot_data(p_value_all, title="portfolio value_train", ylabel="USD", save_image=True,save_dir=self.current_working_dir)
  ...
  def testPolicy(self, symbol="IBM",
    ...
    ut.plot_data(trades, title="transactions_test", ylabel="amount", save_image=True,save_dir=self.current_working_dir)
    ut.plot_data(p_value_all, title="portfolio value_test", ylabel="USD", save_image=True,save_dir=self.current_working_dir)
