...
class QLearner(object):
  ...
  """
    ...
    def __init__(self,
        ...
        self.t_table = np.ones(
            (num_actions * num_states, num_states)) * self.t_table_increment_unit  # state transition,
    ...
    def query(self, s_prime, r):
        ...
        @detail: the core method of the Q-Learner. It keep track
        ...
        tuple is <s, a, s_prime, r>. query() return an integer, which is
        the next action to take. Note that it choose a random action with
        probability rar, and that it update rar according to the decay
        rate radr at each step. During exploration and exploitation each state has its own
        rar[random action rate],Details on the arguments:
        ...
        if rand.uniform(0, 1) <= self.state_rar[self.last_state]:  # exploration
            ...
            assert action < self.num_actions
            self.q_table[self.last_state][self.last_action] = (1.0 - self.alpha) * self.q_table[self.last_state][
                self.last_action] + self.alpha * (
            r + self.gamma * self.q_table[s_prime][np.argmax(self.q_table[s_prime])])  # bellman
        ...
        # dyna ( combine model free and with model learning)
        ...
        while self.dyna > 0:
            ...
            # update R
            self.r_table[self.last_state][self.last_action] = (1 - self.dyna_lr) * self.r_table[self.last_state][
                self.last_action] + self.dyna_lr * r
            ...
            ## dyna update Q table
            self.q_table[dyna_state][dyna_action] = (1.0 - self.alpha) * self.q_table[dyna_state][
                dyna_action] + self.alpha * (dyna_r + self.gamma * self.q_table[state_infer_from_t_table][
                np.argmax(self.q_table[state_infer_from_t_table])])
            self.dyna -= 1
        # end of dyna
    ...
    def load_model(self, table_name="q_learner_tables.pkl"):
        """
      load trained q learner, aka the q table (and t table and r table if dyna is included)
      :param table_name:saved table name
    """
